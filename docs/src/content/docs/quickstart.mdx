...
`bag.Fresh` builds a zero-value double per stage; `bag.Alias` makes one instance answer under both types.
...
In Go, vocabulary is methods on your own types embedding `*mokkit.Chain`. `mokkit.Do` runs the step and
...
func (a Act) Register(address string) uuid.UUID {
	a.Helper()

	return a.Get(func(h mokkit.Host) (uuid.UUID, error) {
		return h.Resolve[*SignupService]().Register(h.Context(), address)
	})
}
...
func (i Inspect) WelcomeEmailSent(toAddress string) Inspect {
	...
	return mokkit.Do(i, func(h mokkit.Host) error {
		if slices.Contains(h.Resolve[*fakeEmail]().sent, toAddress) {
			return nil
		}
...
`i.Helper()` keeps a failure pointing at the test's line; the step is named after the verb, so the
...
func TestRegisteringAUserSendsAWelcomeEmail(t *testing.T) {
	f := composition.Enter[Arrange, Act, Inspect](t)

	// ACT — chains are eager: the verb has run by the time it returns, so an
	// Act verb simply hands back its result.
	id := f.Act().Register("acme@example.com")

	// INSPECT — observe the outcome through your vocabulary.
	f.Inspect().WelcomeEmailSent("acme@example.com")
...
`Enter` opens a stage for the test and hands back a **fixture** typed with the suite's own phases, so
the body reads `f.Arrange()` / `f.Act()` / `f.Inspect()`. A suite usually wraps that one line in a
`newFixture(t)` — see [Building your test vocabulary](/concepts/vocabulary/).
