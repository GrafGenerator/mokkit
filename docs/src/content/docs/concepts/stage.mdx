...
f := setup.Enter[Arrange, Act, Inspect](t)

// Or, when the subject is cheap to build, compose and enter per test in one call.
f := mokkit.Enter[Arrange, Act, Inspect](t, mocks, app)
```

`Enter` opens a stage, registers its release with `t.Cleanup`, and hands back a `mokkit.Fixture` typed
...
<TabItem label="Go">
```go
f.Arrange()   // the suite's Arrange — fails hard
f.Act()       // the suite's Act     — fails hard
f.Inspect()   // the suite's Inspect — fails soft

f.Of[Buyer]()                             // the per-test artifact registry, promoted onto the fixture
mokkit.Resolve[*SignupService](f.Stage)   // resolve straight off the stage
```

`mokkit.Fixture[A, C, I]` is generic over the three phase types, so the phases come back as the suite's
...
[Allure reporter](/guides/reporting-allure/) works.
