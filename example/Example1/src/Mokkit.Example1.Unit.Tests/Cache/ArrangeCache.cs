...
/// <summary>
/// Arrange building blocks for the cache service: build the data to act on and shape what the substituted
/// <see cref="IDistributedCache"/> returns. (<c>GetStringAsync</c>/<c>SetStringAsync</c> are extension
/// methods NSubstitute can't intercept, so we configure the real members <c>GetAsync</c>/<c>SetAsync</c>.)
/// </summary>
