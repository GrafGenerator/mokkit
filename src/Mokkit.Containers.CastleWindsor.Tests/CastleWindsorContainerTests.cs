...
namespace Mokkit.Containers.CastleWindsor.Tests;
...
public class CastleWindsorContainerTests
{
    ...
    {
        var builder = new CastleWindsorContainerBuilder()
            .UseInit(container =>
            {
                container.Register(Component.For<ICalculator>().ImplementedBy<RealCalculator>());
                container.Register(Component.For<CalculatorClient>().LifestyleTransient());
    ...
    public async Task ResolvesMockFromStage_ViaBridge()
    {
        ...
        var windsorBuilder = new CastleWindsorContainerBuilder()
            .UseInit(container =>
            {
                container.Register(Component.For<CalculatorClient>().LifestyleTransient());
                return Task.CompletedTask;
            })
            .UsePreBuild<IMockCollection<object>>((container, mocks) =>
            {
                foreach (var registration in mocks.Registrations)
                {
                    container.ResolveFromStage(registration.InnerType);
                }
